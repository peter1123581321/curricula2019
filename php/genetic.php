<?php
	...
	mysqli_query($db,"truncate genetic;");
	...
	function processCurriculum($key,$curriculum) {
		...
		for ($i=1;$i<=$MAXITERATIONS;$i++) {
			...
			// echo "===== MUTATION\n";
			$mutations = generateMutations($population);
	...
	function insertIterationResult($curriculum,$iteration,$individual) {
		...
		$query = "insert into genetic (curriculum, iteration, individual, textlength, totalscore, cloudscore) values ('".$curriculum["id"]."',$iteration,'".getIndividualAsString($individual)."',$length,$totalScore,$cloudScore);";
	...
	function generateMutations($population) {
		global $MAXMUTATIONS;
		$mutations = array();
		...
		while (sizeof($mutations)<$MAXMUTATIONS) {
			...
			$mutation = generateMutation($selectedIndividualForMutation);
			$mutations[]=$mutation;
		}
		return $mutations;
	}
	...
	function buildPopulation($original_individual) {
		...
		while (sizeof($population)<$POPULATION_SIZE) {
			$generatedRandomIndividual = generateMutation($original_individual);	
	...
	function generateMutation($original_individual) {
		global $courses;
		$indexOfRemovedCourse = rand(0,sizeof($original_individual)-1);
		$mutation = $original_individual;
		$removedCourse = array_splice($mutation,$indexOfRemovedCourse,1);
		...
		while(true) {
			...
			if ($randomCourseId!=$removedCourse && !in_array($randomCourseId,$mutation)) {
