<?php
	...
	mysqli_query($db,"truncate genetic_mod1;");
	...
	function deleteInvalidIndividuals($population,$original_individual,$lengthOriginalIndividual) {
		...
		for ($i=$POPULATION_SIZE;$i<sizeof($population);$i++) {
			if (countNumberOfMutations($population[$i], $original_individual)>5 || getLengthOfIndividual($population[$i])<$lengthOriginalIndividual) {
	...
	function insertIterationResult($curriculum,$iteration,$individual) {
		...
		$query = "insert into genetic_mod1 (curriculum, iteration, individual, textlength, totalscore, cloudscore) values ('".$curriculum["id"]."',$iteration,'".getIndividualAsString($individual)."',$length,$totalScore,$cloudScore);";
	...
	function generateMutation($mutation, $original_individual) {
		...
		while(true) {
			...
			if ($randomCourseId!=$removedCourse && !in_array($randomCourseId,$mutation) && getLengthOfCourse($randomCourseId)>=$lengthRemovedCourse*0.9) {
